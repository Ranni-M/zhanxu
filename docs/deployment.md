...
- MAX_USER_STORAGE_MB限制上传素材，不包括导出文件。目前导出文件随项目删除而回收，需要定期查看磁盘用量。管理页 /api/admin/overview 会直接给出占用与磁盘剩余。
- ALLOW_REGISTRATION=false可以关闭新注册，已有用户仍能登录。
- ADMIN_EMAILS=you@example.com 指定站点管理员（逗号分隔多个），改完重启生效。管理员能删除任何账号的文件、项目与账号，只填自己的邮箱。
