...
- 单元与集成测试共46项通过：海报渲染的骨架/配色/二维码、压缩包解析规则、公开/私密发布、备份与恢复、单文件网页版作品集的转义/分节/图片内嵌/模板配色。
...
- 浏览器完整流程24项通过：注册、创建、图片/两页PDF/WebM/ZIP上传、模块关联、保存刷新、未保存内容恢复、发布、匿名访问、PDF公开控制、私密发布后匿名既打不开也搜不到、切回公开恢复访问、真实PNG/ZIP/作品集PDF/单文件网页下载、首页9个模板示例与分类计数、展示模板页9套模板各配一个不同示例、零填写导入压缩包、游客模式建草稿与登录后迁移、首页功能区的对比滑杆与二维码、工作台与深色模式、390px布局与模板区不重合、360px窄窗口无横向溢出、手机编辑器、减少动态效果、无障碍、撤回。
...
- 导出的单文件网页内嵌全部图片，在无网络、无账号的 file:// 下打开仍能看到完整版式；图片读不到时退回原始地址，不会整页失败。
- 浏览器最终运行无未捕获页面错误。
- 手机端横向溢出专项：320/360/375/390/414五档宽度下，首页、展示模板、我的作品、账号与公开作品页的文档宽度均等于视口宽度；模板区滚动过程中文案与模板卡重合数为0。
