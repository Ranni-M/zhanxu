...
- 浏览器完整流程23项通过：注册、创建、图片/两页PDF/WebM/ZIP上传、模块关联、保存刷新、未保存内容恢复、发布、匿名访问、PDF公开控制、私密发布后匿名既打不开也搜不到、切回公开恢复访问、真实PNG/ZIP/作品集PDF/单文件网页下载、首页9个模板示例与分类计数、展示模板页9套模板各配一个不同示例、零填写导入压缩包、游客模式建草稿与登录后迁移、首页功能区的对比滑杆与二维码、工作台与深色模式、390px布局、手机编辑器、减少动态效果、无障碍、撤回。
